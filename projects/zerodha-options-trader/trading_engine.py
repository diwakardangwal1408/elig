    ...
    def __init__(self, config_file: str = "strategies.json"):
        ...
        if system_settings.trading_mode == "paper":
            ...
            if hasattr(system_settings, 'kite_api_key'):  # For market data
                try:
                    real_api = ZerodhaAPI(system_settings.kite_api_key, system_settings.kite_api_secret)
                    self.api.set_real_api(real_api)
                except ValueError as e:
                    self.logger.warning(f"Could not initialize real API for paper trading: {e}")
        else:
            self.api = ZerodhaAPI(system_settings.kite_api_key, system_settings.kite_api_secret)
