...
def run_auth_page():
    """Launch the authentication page"""
    print("Launching Zerodha Authentication Page...")
    
    try:
        # Run streamlit authentication page
        subprocess.run([
            sys.executable, "-m", "streamlit", "run", "auth_page.py",
            "--server.port", "8502",
            "--server.headless", "true"
        ])
    
    except KeyboardInterrupt:
        print("\n🛑 Authentication page stopped")
    
    except Exception as e:
        print(f"❌ Error launching authentication page: {e}")
...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["setup", "check", "dashboard", "engine", "install", "auth"],
    ...
    if args.command == "install":
        ...
        print("1. Edit .env file with your API key and secret")
        print("2. Run: python main.py auth (to authenticate)")
        print("3. Review and modify strategies.json")
