...
class ZerodhaAPI:
    ...
    def __init__(self, api_key: str, api_secret: str, access_token: Optional[str] = None):
        ...
        self.auth_handler = ZerodhaAuthHandler(api_key, api_secret)
        
        # Try to authenticate
        if access_token:
            self.auth_handler.access_token = access_token
        elif not self.auth_handler.load_saved_tokens():
            raise ValueError("No valid access token found. Please authenticate first.")
        
        self.kite = self.auth_handler.get_authenticated_kite()
        if not self.kite:
            raise ValueError("Failed to get authenticated Kite instance")
    ...
    def authenticate_with_request_token(self, request_token: str) -> Dict[str, Any]:
        """Authenticate using request token from Zerodha login"""
        try:
            session_data = self.auth_handler.generate_session(request_token)
            self.kite = self.auth_handler.get_authenticated_kite()
    ...
    def get_login_url(self) -> str:
        ...
        return self.auth_handler.get_login_url()
